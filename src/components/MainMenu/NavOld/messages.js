...
import $ from "jquery";
import {addChatInChatRoom, getChats, toggleChatComposer} from "../../../redux/chat/actions";
...
class Messages extends Component {
    ...
    getConversations = () => {
        ...
        API.CHAT.getAllChats(obj).then(({status, data}) => {
            console.log(data)
            if (status) {
                let {conversations} = this.state;
                const newStates = {
                    conversations: this.currentPage === 1 ? data : [...conversations, ...data],
                }
    ...
    handleChatRooms = ({target}, chat) => {
        ...
        if (_thisElem.hasClass('contact')) {
            ...
            $(parents.filter(i => $(parents[i]).hasClass('contact'))[0]).find('.cnt-tile').remove();
                        ...
                        <div ref={this.messagesHolder} className="board-body ov-des">
                            {!requestPending && chats.length > 0 ? (
                                conversations.map(chat => (
                                    <MessageListItem key={chat.chat_id} chat={chat}
