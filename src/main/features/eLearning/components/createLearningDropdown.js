...
import { FeaturePermissionEnum, FeaturePermissionEnumList, FeaturesEnum } from '../../../../utils/Shared/enums/featuresEnums';
...
function CreateLearningDropdown() {
  ...
  const permissionItems = items.filter((item) => userPermissions.includes(item.permissionId)).map((x) => x)
