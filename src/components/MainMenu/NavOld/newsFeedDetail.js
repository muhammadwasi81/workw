...
export class Index extends Component {
    ...
    getPostById = () => {
        ...
        API.POSTS.getPostsById(id).then(({status, error, data})=>{
            if(status){
                ...
                {typeof showHead === "undefined" && (
                    <div className="cont-header">
                            <div ref={this.tabMenu} className="menu">
                                <NavLink className="li on"
                                         to={`${this.props.match.url ? this.props.match.url : this.props.match}`}>Post details</NavLink>
                            </div>
                        <span className="ln"/>
                    </div>
                )}
