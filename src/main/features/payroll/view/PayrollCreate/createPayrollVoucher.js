...
const CreatePayrollVoucher = () => {
  ...
  let totalAmount =
    ...
    payrollCalculatedList.reduce(
      (a, b) => a + (b.isChecked ? b.netSalary : 0),
      0
    );
  ...
  useEffect(() => {
    if (success) navigate("/payroll");
  }, [success]);
  const createPayload = () => {
    let payload = {
      ...state,
      total: totalAmount,
      details: payrollCalculatedList
        .filter((item) => item.isChecked)
        .map((item) => ({ ...item, month: 1, year: 1 })),
  ...
  return (
    <div className="createEntryTable">
      ...
      <CreateEntryTable>
        {payrollCalculatedList &&
          payrollCalculatedList.map((item, index) => {
            return (
              <CreateEntryItem key={item.userId} value={item} index={index} />
            );
          })}
      </CreateEntryTable>
      <VoucherFooter amount={totalAmount} handleSubmit={handleSubmit} />
    </div>
  );
};
