...
function Employee() {
	...
	const formDataSubmit = (value, imageId) => {
		...
		if (value.educations) {
			...
			filteredEducation = mergeArrayObjects(
				educationDate,
				educationState
			);
		}
		if (value.experiences) {
			...
			filteredExperience = mergeArrayObjects(
				experienceDate,
				experienceState
			);
		...
		filteredEmergencey[0].contactNo = value.contactNo
			? value.contactNo
			: "";
		setIsFormSubmitted(true);
		const valueClone = (({ address, name, relation, contactNo, ...o }) =>
			o)(value);
		setEmployeeForm({
			...employeeForm,
			...valueClone,
	...
	return (
		<Skeleton
			loading={sharedLoader || gradeLoader || accessRoleLoader}
			active
		>
