...
function AccessRoleComposer(props) {
				...
				if (singleAccessRole.features[i].permissions.length > 0) {
					singleAccessRole.features[i].permissions.map(role => {
						checkedData.push(
							JSON.stringify(role) +
								"_" +
								singleAccessRole.features[i].id
						);
					});
				} else {
					...
					singleAccessRoleObj.name =
						singleAccessRole.features[i].name;
	...
	useEffect(() => {
		if (props.isEdited) {
			...
			if (
				JSON.stringify(finalData).includes(
			...
			>
				<div>
					<S.FormItem
						...
						label={
							<FormLabel>
								{
									administration.accessRole.Drawer.formLabel
										.AccessRoleName
								}
							</FormLabel>
						}
					...
					<S.FormItem
						...
						label={
							<FormLabel>
								{administration.accessRole.Drawer.UserType}
							</FormLabel>
						}
						rules={[
							{
					...
					<S.FormItem
						...
						label={
							<FormLabel>{sharedLabels.description}</FormLabel>
						}
					>
						<FormTextArea
							direction={Direction}
				...
				<Form.Item>
					<Button
						...
						loading={loading}
						disabled={
							props.isDefault || (props.isEdited && isObjEqual)
						}
					>
						{props.isEdited
							? isObjEqual
								? administration.accessRole.Button.accessRole
								: administration.accessRole.Button
										.UpdateAccessRole
