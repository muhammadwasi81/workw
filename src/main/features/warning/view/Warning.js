      ...
      <ContainerHeader>
        ...
        <div className="right-menu" style={{ paddingRight: "10px" }}>
          <div className={isTablet ? "btn-hld CompBtnMobile" : "btn-hld"}>
            <SideDrawer title={warningDictionary.warning} buttonText={warningDictionary.createWarning} isAccessDrawer={false}>
              <Composer />
            </SideDrawer>
          </div>
        </div>
      ...
      <TopBar
        ...
        gridIcons={[
          <div onClick={() => setGrid(false)} className={grid ? "topBarIcon gridIcon" : "topBarIcon gridIcon isActive"}>
            {isTablet ? "" : "List View"} <UnorderedListOutlined style={{ marginLeft: "2px" }} />
          </div>,
          <div onClick={() => setGrid(true)} className={grid ? "topBarIcon gridIcon isActive" : "topBarIcon gridIcon"}>
            {isTablet ? "" : "Table View"} <AppstoreFilled style={{ marginLeft: "2px" }} />
          </div>,
        ]}
      />
