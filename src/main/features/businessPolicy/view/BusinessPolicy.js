...
import { getAllBussinessFeatures, getAllUserTypes } from "../../../../utils/Shared/store/actions";
import "../style/accessrole.css";
import { addAccessRole, getAccessRoleById, getAllAccessRoles, updateAccessRoleById } from "../store/action";
...
function BusinessPolicy() {
  ...
  const { loader: loading, success, accessRoles, singleAccessRole } = useSelector((state) => state.accessRolesSlice);
  ...
  useEffect(() => {
    if (singleAccessRole && singleAccessRole.features && singleAccessRole.features.length > 0) {
