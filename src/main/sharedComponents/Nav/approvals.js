...
class Approvals extends Component {
  ...
  handleApprovalClick = ({ referenceType, reference_id }) => {
    ...
    switch (referenceType) {
      ...
      case 190:
      case 191: //Expense
        window.location.replace(`${STRINGS.ROUTES.EXPENSES}/${reference_id}`);
        break;
      case 117:
      case 118:
      ...
      case 143: //Leave
        ...
        break;
      case 89: //Document
        window.location.replace(`${STRINGS.ROUTES.DOCUMENTS.DEFAULT}?f=app`);
        break;
      case 145:
      case 146:
      case 147: // assets
  ...
  getApprovalDetail = ({ referenceType, details }) => {
    ...
    switch (referenceType) {
      case 113 || 114 || 115 || 116 || 136 || 137 || 138 || 139: //Expense
        return (
          <React.Fragment>
            <div className="text">{JSON.parse(details).category}</div>
      ...
      case 143: //Leave
        return (
          <React.Fragment>
            <div className="text">{JSON.parse(details).leaveType}</div>
            <div className="text">{JSON.parse(details).description}</div>
          </React.Fragment>
        );
      case 89: //Document
  ...
  handleAddRemark(finalRemarks, amount, internalAttacment) {
    ...
    const { remarkStatus, hasAttachment, attachment, attachments } = this.state;
    const remark = {
      approval_id: approval.approval_id,
    ...
    if (hasAttachment) {
      API.FILES.upload([attachment]).then(({ status, error, data }) => {
        if (status) {
          remark.attachments = [{ attachment_id: data[0].attachment_id }];
          this.setState({
            remarkDone: true,
          });
