...
export const calculateAllowance = (allowanceData = [], gradeId = null, basicSalary = 0) => {
    let totalAllowance = 0;
    let totalDeductions = 0;
    let details = [];
    // Filteration of Allowances behalf of Grade, AllowanceUnit & Allowance Type =>
    let filteredAllowance = allowanceData.filter(item => item.gradeId === gradeId);
    ...
    details = [
        ...details,
        ...filterBenefit_inAmount.map(item => ({ allowanceId: item.id, allowance: item.value })),
        ...filterDeduction_inAmount.map(item => ({ allowanceId: item.id, allowance: item.value })),
        ...filterBenefit_inPercent.map(item => ({ allowanceId: item.id, allowance:  (item.value / 100 * basicSalary) })),
        ...filterDeduction_inPercent.map(item => ({ allowanceId: item.id, allowance:  (item.value / 100 * basicSalary) }))
    ]
