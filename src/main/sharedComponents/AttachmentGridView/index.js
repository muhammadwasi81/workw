...
const AttachmentGridView = ({ attachments }) => {
  ...
  let attachmentArr = attachments.length > 4 ? attachments.slice(0, 4) : [...attachments];
  console.log(modelState);
  return (
    <React.Fragment>
      <AttachmentGrid length={attachmentArr.length} className="AttachmentGridView" onClick={() => setmodelState(true)}>
        {attachments.length > 4 && <div className="AttachmentViewShutter">+{attachments.length - 4}</div>}
